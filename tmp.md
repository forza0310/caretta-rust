...
- 单连接级 latency / RTT —— eBPF 程序没采集（[network/caretta-ebpf/src/main.rs](../network/caretta-ebpf/src/main.rs)）
- 重传率 —— `tcp_sendmsg` 不区分原发和重传
- 包数 —— 只有字节
- L7 协议解析 —— 完全没做
- 单 trace/connection 的耗时 —— 只有累计量
...
这是一个**服务 × 时间**的多维时序矩阵，每个 cell 是 10+ 维向量。
...
│  └─ caretta_k8s_events_total        └─ metrics ┘                     │
│        │                                          │                  │
│        ▼                                          ▼                  │
...
- [River](https://github.com/online-ml/river) —— 在线学习
...
6. **不要忽视 caretta 自身的局限**：没 L7、没延迟、没重传 —— 别假装它能做这些；那些场景靠 SigNoz。
