...
// 阶段 1：trait 的方法不能直接 async fn（dyn Trait 不支持），用 async_trait 宏把它
...
pub struct DnsCache {
    ...
    // 线程被挂起。原代码的 resolve_name 在 sync 上下文被 poll 循环（tokio worker
    // 线程）直接调用，DNS 抖动会直接转化为 worker 线程饥饿、metrics 抓取超时。
    //
    // hickory_resolver 是纯 Rust 异步 DNS 解析器，所有 I/O 都是 .await 的——卡的
    // 是 task 而不是线程，tokio 调度器可以自由切换到别的 task。再叠加一层
    // tokio::time::timeout 把单次最长时延钉死在几百毫秒级别，最坏情况也不会让
    // 资源占用累积。
    //
...
impl DnsCache {
    ...
    ///
    /// 这里不再用 `TokioAsyncResolver::tokio_from_system_conf()`：那条 API 会同时丢掉系统
    /// `resolv.conf` 解析出来的 ResolverOpts 中我们想覆盖的字段（attempt timeout、内部
    /// cache 大小）。改成手工调 `system_conf::read_system_conf()` 拿 (cfg, opts) 后再
    /// 把我们的字段写回去——这样 happy path 上 attempt 超时也是 500ms 而不是系统默认的 ~5s，
    /// 配合外层 DNS_LOOKUP_TIMEOUT(800ms) 一起把单次反查最坏时延钉死在亚秒级。
